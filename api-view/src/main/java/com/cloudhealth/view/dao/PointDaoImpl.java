...
public class PointDaoImpl implements PointDao {
	...
	public BigInteger countChildDiff(String maleId, String femaleId, String child, int perpage, int offset,
		...
		if("All".equals(hgmdSelect) && "All".equals(clinvarSelect)) {
			...
			sqlQuery = getSession().createSQLQuery("select count(*) from trioDiff diff LEFT JOIN hg38_hgmd hgmd ON(diff.CHROM = hgmd.CHROM and diff.POS = hgmd.POS and diff.REF = hgmd.REF and diff.ALT= hgmd.ALT) where diff.SampleId=:child and diff.FatherId=:father and diff.MotherId=:mother AND hgmd.CLASS=:class");
			...
			sqlQuery = getSession().createSQLQuery("select count(*) from trioDiff diff LEFT JOIN hg38_hgmd hgmd ON(diff.CHROM = hgmd.CHROM and diff.POS = hgmd.POS and diff.REF = hgmd.REF and diff.ALT= hgmd.ALT) LEFT JOIN vcfanno ON(diff.CHROM = vcfanno.CHROM and diff.POS = vcfanno.START and diff.REF = vcfanno.REF and diff.ALT= vcfanno.ALT) where diff.SampleId=:child and diff.FatherId=:father and diff.MotherId=:mother AND vcfanno.Category=:category AND hgmd.CLASS=:class");
