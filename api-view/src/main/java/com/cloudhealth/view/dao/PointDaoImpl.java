...
public class PointDaoImpl implements PointDao {
	...
	public BigInteger count(String sampleId, String chr, int start, int end,String varAnnoGroupType) {
		if(chr.contains("chr"))
			chr = chr.substring(3);
		SQLQuery query;
		if("ALL".equals(varAnnoGroupType)) {
			...
			query = getSession().createSQLQuery("SELECT count(*) FROM sampleinfor sample INNER JOIN vcfanno ON (sample.CHROM=vcfanno.CHROM AND sample.POS=vcfanno.POS AND sample.REF=vcfanno.REF AND sample.ALT=vcfanno.ALT AND vcfanno.Category = :category) WHERE sample.SAMPLE = :sample AND sample.CHROM = :chr AND sample.POS BETWEEN :start AND :end AND LENGTH(sample.REF)=1 AND LENGTH(sample.ALT)=1");
